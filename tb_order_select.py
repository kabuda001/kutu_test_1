...
class LoadThread(QThread):
    ...
    def handleRow1(self,cdr_files_map,row_data):
        ...
        specification_name_str = row_data.get('商品规格')
        ...
        if not self.is_valid_longest_side(longest_side):
            ...
            return
        # 买家留言
        if not self.is_empty_string(row_data.get('卖家备注')) or not self.is_empty_string(row_data.get('买家留言')):
    ...
    def handleTrackMap(self,cdr_files_map):
        for tracking_number, rows in self.tracking_map.items():
            ...
            if len(rows) > 1:
                ...
                style_list=[]
                for row in rows:
                    specification_name_str = row.get('商品规格')
                    style, longest_side = self.parse_specification_name_str(specification_name_str)
    ...
    def copy_cdr_Multiple(self,row_data,style, longest_side,cdr_file_path,much_longest_side,tracking_number):
        ...
        self.appendCdrRow(row_data,style,longest_side,cdr_excel_path)
        # 创建尺寸的文件
        longest_side_file_name = str(order_num) + "_" + str(longest_side)+ "_" + str(good_nums)
    ...
    def copy_file_with_new_name_nums(self,src_file, dst_dir, good_nums , new_name=None):
        ...
        for idx, num in enumerate(range(0, good_nums), start=0):
            if idx == 0:  # 第一个元素命名为 xxx
                # 构建目标文件名，保持原有扩展名
                ...
                # 找到第一个可用的文件名
                while True:
                    new_target = f"{new_name}_{counter}{file_extension}"
                    # 构建目标文件路径
                    dst_file = os.path.join(dst_dir, new_target)
                    if not os.path.exists(dst_file):
                        shutil.copy(src_file, dst_file)
                        print(f"文件 '{src_file}' 已成功拷贝到 '{dst_file}'.")
                        break
